 ...
 */
 ...
 /* Предложен вариант заполнения через процедуру */

 delimiter //
drop procedure if exists fill_users//
create procedure fill_users( in rec_nums int )
begin
	declare i int default 0;
		
	while i <= rec_nums do
		/* Можно добавить генерацию случайного имени или/и даты */
		insert into users ( name, birthday_at ) values ( 'TEST USER', now() );
		set i = i + 1;
	end while;
end//

call fill_users( 1000000 );